...
function Citation({ c }) {
  ...
  return (
    <div className="flex items-start gap-2 rounded-lg border-l-2 border-indigo-400 bg-indigo-50 px-3 py-2 text-xs text-slate-700">
      <FileText className="mt-0.5 h-3 w-3 shrink-0 text-indigo-500" />
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-0.5">
          <span className="font-semibold text-indigo-700">Page {c.page ?? "?"}</span>
          {score && <span className="rounded-full bg-indigo-200 text-indigo-700 px-1.5">{score}</span>}
          {c.document_name && <span className="rounded-full bg-violet-100 text-violet-700 px-1.5 truncate max-w-[120px]">{c.document_name}</span>}
        </div>
        <p className="text-slate-600 text-[11px] leading-relaxed line-clamp-3">{c.text}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-5 py-4 flex flex-col gap-3">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-xs text-slate-400 mt-1">Answers are grounded with page citations</p>
          </div>
        )}
      ...
      <div className="border-t bg-white px-4 py-3">
        <div className="flex items-end gap-2 rounded-xl border border-slate-300 bg-white px-3 py-2 focus-within:border-indigo-400 focus-within:ring-1 focus-within:ring-indigo-400 transition-all">
          <textarea
            rows={1}
            value={input}
